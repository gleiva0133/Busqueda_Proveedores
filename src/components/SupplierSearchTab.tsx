...
export const SupplierSearchTab: React.FC<SupplierSearchTabProps> = ({
  ...
  const [sharePointUrl, setSharePointUrl] = useState<string>(DEFAULT_SHAREPOINT_URL);
  ...
  const filteredResults = matchedResults.filter(item => {
    ...
    if (colSuppliers === 'WITH_LOCAL') {
      matchesColSuppliers = item.matchedSuppliersCount > 0;
    } else if (colSuppliers === 'NO_LOCAL') {
      matchesColSuppliers = item.matchedSuppliersCount === 0;
    } else if (colSuppliers === 'WITH_AI') {
      matchesColSuppliers = item.matchedIASuppliersCount > 0;
    }
  ...
  // Export ONLY currently filtered results to Excel
  const handleExportExcel = () => {
    if (filteredResults.length === 0) return;

    // Sheet 1: Detailed Analysis of currently filtered items
    const sheet1Data = filteredResults.map(r => ({
      'Num Ítem': r.numItem,
    ...
    XLSX.utils.book_append_sheet(wb, ws1, 'ANALISIS_FILTRADO');
    XLSX.utils.book_append_sheet(wb, ws2, 'BASE_PROVEEDORES');

    XLSX.writeFile(wb, `REPORTE_PROVEEDORES_FILTRADO_${new Date().toISOString().slice(0, 10)}.xlsx`);
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-400">
              Sube tus archivos de Proveedores (CSV/Excel) y Requerimientos de Compra (<code className="text-amber-300">qryPOs_Temp</code>) o vincula tu enlace de SharePoint
            </p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {/* Upload Suppliers */}
            <label className="flex items-center justify-center space-x-2 px-3.5 py-3 bg-slate-900/90 hover:bg-slate-900 border border-slate-700 hover:border-amber-500/50 rounded-xl cursor-pointer text-xs font-medium text-slate-200 transition group shadow-sm">
              <Upload className="w-4 h-4 text-amber-400 group-hover:scale-110 transition-transform shrink-0" />
              <div className="text-left truncate">
                <span className="block font-semibold truncate">1. Base Proveedores</span>
            ...
            {/* SharePoint Link Option */}
            <button
              onClick={() => setIsSharePointModalOpen(true)}
              className="flex items-center justify-center space-x-2 px-3.5 py-3 bg-slate-900/90 hover:bg-slate-900 border border-slate-700 hover:border-sky-500/50 rounded-xl text-xs font-medium text-slate-200 transition group shadow-sm cursor-pointer"
            >
              <LinkIcon className="w-4 h-4 text-sky-400 group-hover:scale-110 transition-transform shrink-0" />
              <div className="text-left truncate">
                <span className="block font-semibold truncate">Vincular SharePoint</span>
                <span className="text-[10px] text-slate-400">Enlace Excel Ecuacorriente</span>
              </div>
            </button>
            ...
            <label className="flex items-center justify-center space-x-2 px-3.5 py-3 bg-slate-900/90 hover:bg-slate-900 border border-slate-700 hover:border-amber-500/50 rounded-xl cursor-pointer text-xs font-medium text-slate-200 transition group shadow-sm">
              <Upload className="w-4 h-4 text-amber-400 group-hover:scale-110 transition-transform shrink-0" />
              <div className="text-left truncate">
                <span className="block font-semibold truncate">2. Requerimientos</span>
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-xl w-full p-6 space-y-4 shadow-2xl relative">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="text-base font-bold text-white">Vincular Archivo de Proveedores SharePoint / OneDrive</h3>
                <p className="text-xs text-slate-400">Carga o abre directamente tu archivo maestro de Ecuacorriente</p>
              </div>
            </div>

            <div className="space-y-2">
              <label className="block text-xs font-medium text-slate-300">Enlace del Archivo Excel en SharePoint:</label>
              <input
                type="text"
                value={sharePointUrl}
                onChange={e => setSharePointUrl(e.target.value)}
                className="w-full px-3 py-2.5 bg-slate-950 border border-slate-700 rounded-xl text-xs text-slate-200 focus:outline-none focus:border-sky-500 font-mono"
                placeholder="https://ecuacorrienteecuador-my.sharepoint.com/..."
              />
            </div>

            <div className="p-3 bg-sky-950/30 border border-sky-800/40 rounded-xl text-xs text-slate-300 space-y-2">
              <div className="font-semibold text-sky-300 flex items-center space-x-1.5">
                <ExternalLink className="w-4 h-4 text-sky-400" />
                <span>Instrucciones de Vinculación Directa:</span>
              </div>
              <p className="text-[11px] text-slate-400 leading-relaxed">
                El enlace corporativo de Ecuacorriente está protegido por credenciales de Microsoft 365. Para mejor compatibilidad:
              </p>
              <ol className="list-decimal list-inside text-[11px] text-slate-400 space-y-1">
                <li>Haz clic en <strong>"Abrir en SharePoint"</strong> para ir al archivo original en la nube.</li>
                <li>Si deseas cargarlo directamente en esta web, descarga una copia local en formato <code className="text-amber-300">.xlsx</code>.</li>
                <li>Súbelo con el botón <strong>"1. Base Proveedores"</strong>.</li>
              </ol>
            </div>
            ...
            <div className="flex items-center justify-end space-x-3 pt-2">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="px-4 py-2 bg-sky-600 hover:bg-sky-500 text-white font-semibold text-xs rounded-xl flex items-center space-x-2 transition shadow-sm"
              >
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-2xl overflow-hidden shadow-sm">
        <div className="p-4 border-b border-slate-700 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
          <div className="flex items-center space-x-3">
          ...
          <div className="flex items-center space-x-3">
            <button
              ...
              className="text-xs text-sky-400 hover:text-sky-300 flex items-center space-x-1 font-medium transition cursor-pointer"
            >
              <Filter className="w-3.5 h-3.5" />
              <span>{showColumnFilters ? 'Ocultar Filtros de Columna' : 'Mostrar Filtros por Columna'}</span>
            </button>
            <span className="text-xs text-slate-400 hidden lg:inline">
              Haz clic en una fila para desplegar detalles
            </span>
          </div>
        </div>
        ...
        {filteredResults.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-900/90 text-[11px] font-semibold text-slate-300 uppercase tracking-wider border-b border-slate-700">
                  <th className="py-3 px-4 w-36">Item / PO</th>
                  <th className="py-3 px-4">Material & Especificaciones</th>
                  <th className="py-3 px-4 w-52">Categoría / Depto</th>
                  <th className="py-3 px-4 w-40">Comprador</th>
                  <th className="py-3 px-4 w-48 text-center">Proveedores Encontrados</th>
                  <th className="py-3 px-4 w-28 text-right">Acción</th>
                </tr>

                {/* Per-Column Filter Input Row */}
                {showColumnFilters && (
                  <tr className="bg-slate-950/70 border-b border-slate-700/80 text-xs">
                    <th className="p-2">
                      <input
                        type="text"
                        placeholder="Filtrar # o PO..."
                        value={colItem}
                    ...
                    <th className="p-2">
                      <select
                        value={colSuppliers}
                        onChange={e => setColSuppliers(e.target.value)}
                        className="w-full px-2 py-1 bg-slate-900 border border-slate-700 rounded-lg text-xs text-slate-200 focus:outline-none focus:border-amber-500 font-normal"
                      >
                        <option value="ALL">Todos los Estados</option>
                        <option value="WITH_LOCAL">Con Proveedores Locales</option>
                        <option value="NO_LOCAL">Sin Coincidencia Local</option>
                        <option value="WITH_AI">Con Proveedores IA</option>
                      </select>
                    </th>
